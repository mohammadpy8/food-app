...
import { password, pardakhtYekja } from "../../../utils";
...
describe("ورود به ام حامی", () => {
  ...
  it("چک کردن مقدار برای پرداخت یکجا", () => {
    Cypress.on("uncaught:exception", (err, runnable) => false);
    cy.wait(500);

    cy.login(pardakhtYekja.username, password); /// وارد کردن اطلاعات
    cy.wait(500);
    ...
    cy.then(() => {
      cy.getAllLocalStorage()
        .then((result) => {
          ...
          natioanalCode = getNationalCode.nationalCode; /// در یافت کد ملی
        ...
        .then(() => {
          ...
          cy.wait(500);
          cy.get("label").contains("پرداخت یکجا").click(); //// رفتن به فرایند پرداخت یکجا
          cy.wait(500);
          cy.get("input[max='10']").then((input) => {
            cy.wait(2000);
            ////// عوض کردن مقدار ماهانه با استفاده از یه کامند که  در ابتدای این قایل نوشته شده اسنت نوشته شده است
            cy.inputChange(input, "7");
            cy.get("span").contains("مشاهده حامیان ").click();
            cy.wait(500);
            cy.get("label[style='font-size: 15px;']");
            cy.get("label")
              .contains("رزگار حسین")
              .invoke("text")
              .then((text) => {
