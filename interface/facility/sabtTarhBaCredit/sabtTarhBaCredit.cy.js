...
import { mainInfo, infoTrahHamgani } from "../../../info";

describe("ثبت طرح با دادن اعتبار", () => {
  let token;
  ...
  it("ثبت طرح", () => {
    ...
    cy.get("label").contains("ام حامی").scrollIntoView({ easing: "linear", timeout: 5000 });
    cy.wait(100);
    cy.get(
    ...
    cy.get("textarea").click().type(infoTrahHamgani.deatils); ///// پر کردن بخش توضیخات
    cy.wait(500);
      ...
      cy.getAllLocalStorage()
        .then((result) => {
          const getToken = JSON.parse(
            result["https://stage12.qhami.com"].mresalatPwa
          );
        ...
        .then(() => {
          cy.request({
            ...
            url: "http://stage12api.qhami.com/mresalat/supporter-plan/find-by-username-and-nationalcode",
            headers: {
