...
import { mainInfo, infoTarhWithUser } from "../../../info";

describe("ثبت طرح با اعضا", () => {
  let token;
  ...
  it("ثبت طرح", () => {
    ...
    ).as("checkUser"); /// گرفتن api  مربوط به حامی که باید ثبت شود
    ...
    cy.get("#title").click().type(infoTarhWithUser.title); //// اسم طرح
    cy.wait(500);
    ...
    cy.get("label")
      ...
      .invoke("text")
      .then((text) => {
        expect(text).to.equal(infoTarhWithUser.errorDate);
      }); //// صبر کردن برای انتخاب تاریخ

    cy.pause();

    cy.get("textarea").click().type(infoTarhWithUser.deatils); ///// پر کردن بخش توضیخات
    cy.wait(500);
    ...
    cy.get(".supporterCeateLabelChip > .form_input_container").each(
      ...
      (element, index) => {
        if (index === 0) {
          cy.wrap(element).type(infoTarhWithUser.price);
        }
      }
    );
    cy.wait(500);
    ...
    cy.wait("@checkUser").then((res) => {
      ...
      cy.get(".input-wrapper").each(($el, index) => {
        //// وارد کردن مبلغ
        if (index === 2) {
          cy.wrap($el).type(infoTarhWithUser.totalPrice); //// وارد کردن مبلغ کل
        }
      });
      cy.wait(500);
      ...
      cy.get(".input-wrapper").each(($el, index) => {
        //////////// اعطای مبلغ بع فرد
        if (index === 3) {
          cy.wrap($el).type(infoTarhWithUser.creditPrice); ///// وارد کردن مبلع متناسب با اطلاعات وارد شده
        }
      });
      cy.wait(500);
      ...
      cy.wait(500);
      cy.get("button").contains("مشاهده لیست اعضای گروه حامی").click(); ///// رفتن به قسمت ثبت اعضا برای طرح
      cy.get(".form_input_container").each(($el, index) => {
        cy.wrap($el).type(
          index === 0
            ? infoTarhWithUser.userHami
            : infoTarhWithUser.nationalCodeUserHami
        ); ////// وارد کردن اطلاعات یه عضو
      });
      cy.wait(500);
      cy.get('.mt-4 > div > .sc-beyTiQ').click();
      cy.wait(500);
      cy.wait("@checkUser").then((res) => {
        const fullName = res.response.body.fullName; //// گرفتن اسم عضو پست شده
        cy.get(".wizard-container > .mt-4 > div > .sc-guDMob")
          .invoke("text")
          .then((text) => {
            expect(fullName).to.equal(text); //// چک کردن اطلاعات عضو با api
          });
      });
      cy.wait(500);
      cy.get(".input-wrapper")
      ...
      cy.get(".input-wrapper").each(($el, index) => {
        if (index === 5) {
          cy.wrap($el).type(infoTarhWithUser.priceUser);
        }
      });
      cy.wait(500);
      cy.get("label[id='creditUpToAmountSP']")
        .invoke("text")
        .then((text) => {
          const getPrice = text.split(" ");
          const priceFinaal = Number(getPrice[0].split(",").join(""));
