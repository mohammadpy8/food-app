...
import { password, sabtNotUsers, detailsSabtTarh } from "../../../info";
import { numberWithCommas } from "../../../utils";

/* -------------------------------------------------------------------------- */
/*                          ثبت کردن طرح بدون هیچ عضو                         */
/* -------------------------------------------------------------------------- */

describe("ثبت طرح بدون یوزر", () => {
  let token;
  ...
  it("ثبت طرح بدون اعضا برای استفاده از حامیان", () => {
    ...
    cy.login(sabtNotUsers.username, password); /// وارد کردن اطلاعات
    cy.wait(500);
    ...
    cy.get("#title").click().type(detailsSabtTarh.titleTarh); //// اسم طرح
    cy.wait(500);
    ...
    cy.get("label")
      ...
      .invoke("text")
      .then((text) => {
        expect(text).to.equal(detailsSabtTarh.errorDate);
      }); //// صبر کردن برای انتخاب تاریخ

    cy.pause();

    cy.get(".sc-kEbfvU").click().type(detailsSabtTarh.detailsTarh); ///// پر کردن بخش توضیخات
    cy.wait(500);
    ...
    cy.get(".supporterCeateLabelChip > .form_input_container").each(
      ...
      (element, index) => {
        if (index === 0) {
          cy.wrap(element).type(detailsSabtTarh.price);
        }
      }
    );
    cy.wait(500);
    ...
    cy.wait(500);
    cy.wait(2000);
    cy.get("input[text='تومان']").then((el) => {
      cy.wrap(el).clear();
    });
    cy.wait(500);
    cy.get("input[text='تومان']").then((el) => {
      cy.wrap(el).type(detailsSabtTarh.priceD);
    });
    cy.wait(500);
    ...
    cy.get(".input-wrapper").each(($el, index) => {
      cy.wrap($el).type(
        index === 0 ? sabtNotUsers.userHami : sabtNotUsers.nationalCodeHami
      ...
      cy.getAllLocalStorage()
        ...
        .then(() => {
          cy.request({
            ...
            body: {
              username: sabtNotUsers.userHami,
              nationalCode: sabtNotUsers.nationalCodeHami,
            },
          }).then((res) => {
            ...
            cy.get(".input-wrapper").each(($el, index) => {
              //// وارد کردن مبلغ
              if (index === 2) {
                cy.wrap($el).type(detailsSabtTarh.totalPrice); //// وارد کردن مبلغ کل
              }
            });
            cy.wait(500);
            ...
            cy.get(".input-wrapper").each(($el, index) => {
              //////////// اعطای مبلغ بع فرد
              if (index === 3) {
                cy.wrap($el).type(detailsSabtTarh.creditPrice); ///// وارد کردن مبلع متناسب با اطلاعات وارد شده
              }
            });
            cy.wait(500);
